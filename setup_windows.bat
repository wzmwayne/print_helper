...
echo from reportlab.lib.pagesizes import letter, A4
echo from reportlab.lib.utils import ImageReader
echo import io
...
echo def convert_to_pdf(file_path, output_path):
echo     """将各种文件格式转换为PDF"""
echo     try:
echo         file_ext = os.path.splitext(file_path)[1].lower()
echo         
echo         if file_ext == '.pdf':
echo             # 已经是PDF，直接复制
echo             import shutil
echo             shutil.copy2(file_path, output_path)
echo             return True
echo             
echo         elif file_ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.webp']:
echo             # 图片转PDF
echo             img = Image.open(file_path)
echo             if img.mode != 'RGB':
echo                 img = img.convert('RGB')
echo             
echo             # 计算适合的页面大小
echo             img_width, img_height = img.size
echo             page_width, page_height = A4
echo             
echo             # 如果图片太大，缩放到适合页面
echo             if img.width > page_width or img_height > page_height:
echo                 ratio = min(page_width / img_width, page_height / img_height)
echo                 new_width = int(img_width * ratio)
echo                 new_height = int(img_height * ratio)
echo                 img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
echo                 img_width, img_height = new_width, new_height
echo             
echo             # 创建PDF
echo             c = canvas.Canvas(output_path, pagesize=A4)
echo             
echo             # 居中放置图片
echo             x = (page_width - img_width) / 2
echo             y = (page_height - img_height) / 2
echo             c.drawImage(ImageReader(img), x, y, img_width, img_height)
echo             c.save()
echo             return True
echo             
echo         elif file_ext == '.docx':
echo             # Word文档转PDF
echo             doc = docx.Document(file_path)
echo             c = canvas.Canvas(output_path, pagesize=A4)
echo             page_width, page_height = A4
echo             margin = 50
echo             y_position = page_height - margin
echo             line_height = 14
echo             
echo             for paragraph in doc.paragraphs:
echo                 text = paragraph.text
echo                 if not text.strip():
echo                     y_position -= line_height
echo                     continue
echo                     
echo                 # 分行处理
echo                 words = text.split()
echo                 line = ""
echo                 for word in words:
echo                     test_line = line + word + " "
echo                     if c.stringWidth(test_line, "Helvetica", 12) < page_width - 2 * margin:
echo                         line = test_line
echo                     else:
echo                         if line:
echo                             c.drawString(margin, y_position, line.strip())
echo                             y_position -= line_height
echo                         line = word + " "
echo                         
echo                         if y_position < margin:
echo                             c.showPage()
echo                             y_position = page_height - margin
echo                 
echo                 if line:
...
echo                     y_position = page_height - margin
echo                     
echo                 y_position -= line_height // 2  # 段落间距
echo                 
echo                 if y_position < margin:
echo                     c.showPage()
echo                     y_position = page_height - margin
...
echo             return True
echo             
echo         elif file_ext in ['.txt', '.md', '.html', '.htm', '.odt']:
echo             # 文本文件转PDF
echo             with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
echo                 text = f.read()
echo             
echo             c = canvas.Canvas(output_path, pagesize=A4)
echo             page_width, page_height = A4
...
echo                 # 分行长行
echo                 words = line.split()
echo                 current_line = ""
echo                 for word in words:
echo                     test_line = current_line + word + " "
echo                     if c.stringWidth(test_line, "Helvetica", 12) < page_width - 2 * margin:
echo                         current_line = test_line
echo                     else:
echo                         if current_line:
echo                             c.drawString(margin, y_position, current_line.strip())
echo                             y_position -= line_height
echo                         current_line = word + " "
echo                         
echo                         if y_position < margin:
echo                             c.showPage()
echo                             y_position = page_height - margin
...
echo                     y_position = page_height - margin
echo                 
echo                 y_position -= line_height // 2  # 行间距
echo                 
echo                 if y_position < margin:
echo                     c.showPage()
echo                     y_position = page_height - margin
...
echo             
echo         else:
echo             # 尝试使用PyMuPDF处理其他格式
echo             try:
echo                 doc = fitz.open(file_path)
echo                 pdf_bytes = doc.convert_to_pdf()
echo                 with open(output_path, 'wb') as f:
echo                     f.write(pdf_bytes)
echo                 doc.close()
echo                 return True
echo             except:
echo                 return False
...
echo         print(f"转换错误: {str(e)}")
echo         return False

echo def allowed_file(filename):
echo     """检查文件类型"""
echo     if not '.' in filename:
echo         return False
echo     
echo     # 支持的文件类型
echo     supported_extensions = {
echo         # 文档类型
echo         '.pdf', '.doc', '.docx', '.txt', '.rtf',
echo         # 图片类型
echo         '.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.webp',
echo         # 其他格式
echo         '.html', '.htm', '.md', '.odt', '.ods', '.odp'
echo     }
echo     
echo     return filename.lower().endswith(tuple(supported_extensions))

echo def process_pdf_async(file_path, task_id):
echo     """异步处理文件，转换为PDF后分离奇偶页"""
echo     try:
echo         processing_progress["phase"] = "processing"
echo         processing_progress["status"] = "processing"
echo         processing_progress["current"] = 0
echo         processing_progress["message"] = "正在转换文件格式..."
echo         
echo         # 创建临时目录
echo         temp_dir = tempfile.mkdtemp()
echo         pdf_path = os.path.join(temp_dir, 'converted.pdf')
echo         
echo         # 转换文件为PDF
echo         if not convert_to_pdf(file_path, pdf_path):
echo             processing_progress["status"] = "error"
echo             processing_progress["message"] = "文件转换失败，不支持此格式"
echo             processing_progress["phase"] = "error"
echo             return
echo         
echo         processing_progress["message"] = "正在处理PDF页面..."
echo         
echo         # 读取转换后的PDF文件
...
echo             processing_progress["status"] = "error"
echo             processing_progress["message"] = "转换后的PDF文件没有页面内容"
echo             processing_progress["phase"] = "error"
echo             return
...
echo             "converted_pdf": pdf_path  # 保存转换后的PDF路径
echo         }
echo         
echo     except Exception as e:
echo         processing_progress["status"] = "error"
...
echo         <h1>📄 手动双面助手</h1>
echo         <p class="subtitle">上传文件（PDF/图片/文档），自动转换为PDF并分离奇偶页</p>
echo         
echo         <div class="upload-area" id="upload-area">
echo             <div class="upload-icon">📤</div>
echo             <h3>拖放文件或点击选择</h3>
echo             <p>支持PDF、图片(JPG/PNG/GIF等)、文档(Word/TXT等)，最大500MB</p>
echo             <button class="browse-btn" id="browse-btn">选择文件</button>
echo             <input type="file" id="file-input" accept=".pdf,.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp,.doc,.docx,.txt,.rtf,.html,.htm,.md,.odt,.ods,.odp">
echo         </div>
echo         
...
echo                 
echo                 // 检查文件类型
echo                 const fileName = file.name.toLowerCase();
echo                 const supportedTypes = [
echo                     '.pdf', '.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.webp',
echo                     '.doc', '.docx', '.txt', '.rtf', '.html', '.htm', '.md', '.odt', '.ods', '.odp'
echo                 ];
echo                 
echo                 let isSupported = false;
echo                 for (let type of supportedTypes) {
echo                     if (fileName.endsWith(type)) {
echo                         isSupported = true;
echo                         break;
echo                     }
echo                 }
echo                 
echo                 if (!isSupported) {
echo                     alert('不支持的文件类型！请选择PDF、图片或文档文件。');
echo                     return;
echo                 }
echo                 
...
echo                 statusText.textContent = '开始处理文件...';
echo                 
echo                 try {
...
echo                                 statusText.textContent = '文件上传成功，正在转换为PDF并处理...';
echo                                 
echo                                 // 显示页面预览
...
echo             return jsonify({'success': False, 'error': '不支持的文件类型'})
echo         
echo         # 保存上传的文件
echo         temp_dir = tempfile.mkdtemp()
